export function Api ($rootScope, $resource, config) {
  ...
  return {
    ...
    SingularDelete: $resource(server + '/transactions/:transactionId', { transactionId: '@_id' }, {
    ...
    EncryptionKeys: $resource(server + '/encryption-keys/:keyId', { keyId: '@_id' }, {
      update: { method: 'PUT' },
      generate: {
        method: 'POST',
        url: server + '/encryption-keys/generate'
      }
    }),
