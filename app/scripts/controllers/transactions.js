...
import transactionDeleteModal from '~/views/transactionsDeleteModal.html'
import { TransactionsRerunModalCtrl } from './transactionsRerunModal'
import { TransactionsDeleteModalCtrl } from './transactionsDeleteModal'
...
export function TransactionsCtrl ($scope, $uibModal, $location, $timeout, $interval, Api, Alerting) {
  ...
  $scope.refreshTransactionsList = function () {
    ...
    if ($scope.ngError.hasErrors === false) {
      ...
      } else if ($scope.bulkDelete) {
        // do API call only for 'bulkrerun' properties
        const returnFilters = $scope.returnFilters()
        // add filterRepresentation to only return bulkrerun properties
        returnFilters.filterRepresentation = 'bulkdelete'

        // set filter limit to default defaultBulkRerunLimit
        returnFilters.filterLimit = $scope.defaultBulkRerunLimit

        // set bulkDeleteActive true to show delete information
        $scope.bulkDeleteActive = true
        Api.Transactions.get(returnFilters, refreshSuccess, refreshError)
      } else {
  ...
  /****************************************************/
  /**         Transactions Delete Functions           **/
  /****************************************************/
  $scope.bulkDeleteInitiate = function () {
    $scope.bulkDelete = true
    $scope.deleteSelectedTransactions()
  }
  ...
  $scope.bulkDeleteCancel = function () {
    $scope.bulkDelete = false
    $scope.bulkDeleteActive = false
    $scope.refreshTransactionsList()
  }
  ...
  $scope.deleteSelectedTransactions = function () {
    ...
    const deleteTransactionsSelected = $scope.deleteTransactionsSelected
    $uibModal.open({
      template: transactionDeleteModal,
      controller: TransactionsDeleteModalCtrl,
      scope: $scope,
      resolve: {
        transactionsSelected: function () {
          return transactionsSelected
        },
        deleteTransactionsSelected: function () {
          return deleteTransactionsSelected
        }
      }
    })
  }
  ...
  $scope.$on('transactionDeleteSuccess', function () {
    $scope.bulkDeleteActive = false
    $scope.bulkDelete = false
    $scope.checkbox.checkAll = false
    $scope.refreshTransactionsList()
